...
def get_vendor_names():
    sql = f"SELECT vendor_name FROM pgp.vendors"
...
def add_new_product_to_products(vendor_id,product_name,product_description,product_quantity,product_price):
    sql = f"INSERT INTO pgp.products (product_id,vendor_id,product_name,product_description,inventory_stock,price) " \
          f"VALUES (default,{vendor_id},'{product_name}','{product_description}',{product_quantity},{product_price})"
    return executeQueryAndCommit(sql)

def get_vendor_id_with_vendor_name(vendor_name):
    sql = f"SELECT vendor_id FROM pgp.vendors WHERE vendor_name = '{vendor_name}'"
    return executeQueryAndReturnResultNOCOLUMNNAME(sql)[0][0]

def increase_inventory(quantity, product_name):
    sql = f"UPDATE pgp.products SET inventory_stock = inventory_stock + {quantity} WHERE product_name = '{product_name}';"
    return executeQueryAndCommit(sql)


def decrease_inventory(quantity, product_name):
    sql = f"UPDATE pgp.products SET inventory_stock = inventory_stock - {quantity} WHERE product_name = '{product_name}';"
    return executeQueryAndCommit(sql)
